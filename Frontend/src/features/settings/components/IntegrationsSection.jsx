...
const IntegrationsSection = () => {
  ...
  const [integrations, setIntegrations] = useState(currentTenant?.integrations || []);
  ...
  const handleAuthChange = (index, field, value) => {
    const newIntegrations = [...integrations];
    newIntegrations[index].auth = { ...newIntegrations[index].auth, [field]: value };
    setIntegrations(newIntegrations);
  };
    ...
    <div className="bg-(--color-bg-surface) border border-(--color-secondary) rounded-[14px] p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-(--color-primary) text-white px-3 py-1.5 rounded-lg text-sm hover:opacity-90 transition-opacity"
        >
          <Plus size={16} /> Add Integration
        </button>
      </div>

      {error && <div className="text-red-500 text-sm bg-red-100/10 p-3 rounded-lg border border-red-500/20">{error}</div>}
      {success && <div className="text-green-500 text-sm bg-green-100/10 p-3 rounded-lg border border-green-500/20">{success}</div>}
      ...
      {integrations.length === 0 ? (
        ...
        <div className="space-y-6">
          {integrations.map((integration, iIndex) => (
            <div key={iIndex} className="border border-(--color-secondary) rounded-[10px] p-5 space-y-4">
              <div className="flex justify-between items-start">
                <h3 className="font-medium text-sm">Integration {iIndex + 1}</h3>
                <button onClick={() => handleRemoveIntegration(iIndex)} className="text-red-500 hover:bg-red-500/10 p-1 rounded">
                  <Trash2 size={16} />
                </button>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs mb-1 text-(--color-text-secondary)">Name</label>
                  <input
                    value={integration.name}
                    onChange={(e) => handleIntegrationChange(iIndex, 'name', e.target.value)}
                ...
                <div>
                  <label className="block text-xs mb-1 text-(--color-text-secondary)">Base URL</label>
                  <input
                    value={integration.baseUrl}
                    onChange={(e) => handleIntegrationChange(iIndex, 'baseUrl', e.target.value)}
              ...
              <div className="bg-(--color-bg-default) p-4 rounded-lg border border-(--color-secondary) space-y-3">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-[10px] uppercase mb-1 text-(--color-text-secondary)">Auth Type</label>
                    <select
                      value={integration.auth.type}
                      onChange={(e) => handleAuthChange(iIndex, 'type', e.target.value)}
                  ...
                  {integration.auth.type !== 'none' && (
                    <>
                      <div>
                        <label className="block text-[10px] uppercase mb-1 text-(--color-text-secondary)">Key</label>
                        <input
                          type="password"
                          value={integration.auth.key}
                          onChange={(e) => handleAuthChange(iIndex, 'key', e.target.value)}
                          placeholder={integration.auth.key ? "Encrypted (Edit to change)" : "Enter secret key"}
                          className="w-full bg-(--color-bg-subtle) border border-(--color-secondary) rounded-[8px] px-3 py-1.5 text-sm"
                        />
                      </div>
                      {integration.auth.type === 'apiKey' && (
                        <div>
                          <label className="block text-[10px] uppercase mb-1 text-(--color-text-secondary)">Header Name</label>
                          <input
                            value={integration.auth.headerName}
                            onChange={(e) => handleAuthChange(iIndex, 'headerName', e.target.value)}
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => handleAddEndpoint(iIndex)} className="text-xs text-(--color-primary) flex items-center gap-1 hover:underline">
                    <Plus size={12} /> Add Endpoint
                  </button>
                </div>
                
                {integration.endpoints.map((endpoint, eIndex) => (
                  <div key={eIndex} className="bg-(--color-bg-default) p-4 rounded-lg border border-(--color-secondary) space-y-3 relative">
                    <button 
                    ...
                    <div className="grid md:grid-cols-3 gap-3 pr-6">
                      <div>
                        <label className="block text-[10px] uppercase mb-1 text-(--color-text-secondary)">Tool Name (Unique)</label>
                        <input
                          value={endpoint.name}
                          onChange={(e) => handleEndpointChange(iIndex, eIndex, 'name', e.target.value)}
                      ...
                      <div>
                        <label className="block text-[10px] uppercase mb-1 text-(--color-text-secondary)">Method</label>
                        <select
                          value={endpoint.method}
                          onChange={(e) => handleEndpointChange(iIndex, eIndex, 'method', e.target.value)}
                          className="w-full bg-(--color-bg-subtle) border border-(--color-secondary) rounded-[6px] px-2 py-1 text-xs"
                        >
                      ...
                      <div>
                        <label className="block text-[10px] uppercase mb-1 text-(--color-text-secondary)">Path</label>
                        <input
                          value={endpoint.path}
                          onChange={(e) => handleEndpointChange(iIndex, eIndex, 'path', e.target.value)}
                    ...
                    <div>
                      <label className="block text-[10px] uppercase mb-1 text-(--color-text-secondary)">Description for AI</label>
                      <input
                        value={endpoint.description}
                        onChange={(e) => handleEndpointChange(iIndex, eIndex, 'description', e.target.value)}
                    ...
                    <div className="pt-2 border-t border-(--color-secondary)">
                      <div className="flex justify-between items-center mb-2">
                        <label className="block text-[10px] uppercase text-(--color-text-secondary)">Parameters</label>
                        <button onClick={() => handleAddParam(iIndex, eIndex)} className="text-[10px] text-(--color-primary) flex items-center hover:underline">
                        ...
                        <div className="space-y-2">
                          {endpoint.params.map((param, pIndex) => (
                            <div key={pIndex} className="flex items-center gap-2">
                              <input
                                value={param.name}
                                onChange={(e) => handleParamChange(iIndex, eIndex, pIndex, 'name', e.target.value)}
                                placeholder="Param name"
                                className="flex-1 bg-(--color-bg-subtle) border border-(--color-secondary) rounded-[4px] px-2 py-1 text-xs"
                              />
                              <select
                                value={param.type}
                                onChange={(e) => handleParamChange(iIndex, eIndex, pIndex, 'type', e.target.value)}
                              ...
                              <label className="flex items-center gap-1 text-[10px]">
                                <input
                                  ...
                                  onChange={(e) => handleParamChange(iIndex, eIndex, pIndex, 'required', e.target.checked)}
                                />
                                Req.
                              </label>
                              <button onClick={() => handleRemoveParam(iIndex, eIndex, pIndex)} className="text-red-500 p-1">
                                <Trash2 size={12}/>
                              </button>
                            </div>
                          ))}
