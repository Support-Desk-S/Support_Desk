...
const ChatWidget = ({ apiKey }) => {
  ...
  const [customerEmail, setCustomerEmail] = useState(
    () => localStorage.getItem(`sd_widget_email_${apiKey}`) || "",
  );
  const [emailPrompted, setEmailPrompted] = useState(false);
  const [ticketId, setTicketId] = useState(
    () => localStorage.getItem(`sd_widget_ticket_${apiKey}`) || null,
  );
  ...
  useEffect(() => {
    if (customerEmail && isOpen && apiKey) {
      getCustomerTicketsApi(apiKey, customerEmail)
        .then((res) => {
      ...
      try {
        ...
        setMessages((prev) => {
          ...
          if (
            lastMsg &&
            lastPrevMsg &&
            (lastMsg.sender !== lastPrevMsg.sender ||
              lastMsg.message !== lastPrevMsg.message ||
              lastMsg._id !== lastPrevMsg._id)
          ) {
            return [
              {
                _id: "welcome",
    ...
    if (!customerEmail && !emailPrompted) {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          sender: "ai",
          message:
            "Before I can help you with that, could you please provide your email address so we can reach you if we get disconnected?",
          createdAt: new Date().toISOString(),
        },
      ]);
    ...
    if (!customerEmail && emailPrompted) {
      ...
      setMessages((prev) => [
        ...prev,
        {
          _id: Date.now().toString(),
          sender: "ai",
          message: "Thanks! Let me check on that for you...",
          createdAt: new Date().toISOString(),
        },
      ]);
      ...
      try {
        setSending(true);
        const res = await sendInitialMessageApi(apiKey, {
        ...
        if (err.response?.status === 404) {
          ...
          setMessages((prev) => [
            ...prev,
            {
              ...
              sender: "ai",
              message:
                "The current ticket session was closed or not found. Please start a new chat.",
              createdAt: new Date().toISOString(),
            },
          ]);
        } else {
  ...
  return (
    <div
      ...
      style={{
        width: config?.width ? config.width + "px" : "350px",
    ...
    >
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-xs opacity-90 mt-0.5">
              {config.subtitle || "We typically reply in a few minutes"}
            </p>
          </div>
        </div>
      ...
      {activeTab === "current" ? (
        <>
          {/* Messages Area */}
          <div className="flex-1 min-h-0 overflow-y-auto p-4 bg-[#f9fafb] space-y-4">
                ...
                >
                  {!isCustomer && (
                    <div
                      ...
                      style={{
                        backgroundColor:
                          msg.sender === "ai" ? "#f3f4f6" : config.primaryColor,
                        color:
                          msg.sender === "ai" ? config.primaryColor : "#fff",
                      }}
                    >
                      {msg.sender === "ai" ? (
                        <Sparkles size={14} />
                  ...
                  <div
                    ...
                    style={
                      isCustomer ? { backgroundColor: config.primaryColor } : {}
                    }
                  >
                    {msg.message.split("\n").map((line, i) => (
                      <p key={i} className="min-h-3.5">
                        {line}
                      </p>
                    ))}
                  </div>
          ...
          <div className="p-3 bg-white border-t border-gray-100 shrink-0">
            {ticketId &&
            previousTickets.find((t) => t._id === ticketId)?.status ===
              "resolved" ? (
            ...
            previousTickets.map((ticket) => (
                  ...
                  setMessages([
                    {
                      _id: "welcome",
                      sender: "ai",
                      message: config?.welcomeMessage || "Loading...",
                      createdAt: new Date().toISOString(),
                    },
                  ]);
                }}
              >
                <div className="flex justify-between items-start mb-1">
                  ...
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full shrink-0 ${ticket.status === "resolved" ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"}`}
                  >
                    {ticket.status}
                  </span>
                </div>
