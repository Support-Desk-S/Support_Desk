(function () {
  ...
  if (!apiKey) {
    console.error(
      "[SupportDesk] Missing data-api-key attribute on script tag.",
    );
    return;
  }
  ...
  button.addEventListener("click", () => {
    ...
    if (iframe.contentWindow) {
      iframe.contentWindow.postMessage(
        JSON.stringify({ type: "SUPPORT_DESK_TOGGLE", isOpen }),
        "*",
      );
    }
  ...
  const toCssUnit = (val) => {
    ...
    if (typeof val === "string") {
      // if pure number string → convert to px
      if (/^\d+$/.test(val)) return `${val}px`;
      return val; // already px / % / etc
    }
    ...
    try {
      ...
      if (data.type === "SUPPORT_DESK_CONFIG") {
        ...
        if (data.width !== undefined) {
          const w = toCssUnit(data.width);
          if (w) iframe.style.width = w;
        }

        if (data.height !== undefined) {
          const h = toCssUnit(data.height);
          if (h) iframe.style.height = h;
        }

        if (data.radius !== undefined) {
          const r = toCssUnit(data.radius);
          if (r !== null) iframe.style.borderRadius = r;
        }
